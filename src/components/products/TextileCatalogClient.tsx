...
import { textileProducts, resolveTextileImage, type TextileProduct } from '@/lib/textile-products';
import { Grid3X3, List, Search, SlidersHorizontal } from 'lucide-react';

type ViewMode = 'grid' | 'list';

const forms = ['All', 'Bedding', 'Bath', 'Dining'];

function ProductActionLinks({ product }: { product: TextileProduct }) {
...
export default function TextileCatalogClient() {
  ...
  return (
    <div className="space-y-8">
      <div className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-4">
        <label htmlFor="textile-search" className="flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 md:col-span-2">
        ...
        <div>
          <label htmlFor="textile-form-filter" className="sr-only">
            Filter by form
          </label>
          <select
            id="textile-form-filter"
            name="form"
            value={selectedForm}
      ...
      {products.length === 0 && (
        <Card className="border border-slate-200 bg-white p-10 text-center text-slate-600">
          No textile products match your current filters.
        </Card>
      )}
    </div>
  );
